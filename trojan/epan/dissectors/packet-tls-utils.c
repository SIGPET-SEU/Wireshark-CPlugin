...
tls13_key_update(SslDecryptSession *ssl, bool is_from_server)
{
    ...
    ssl_debug_printf("%s tls13_change_key\n", G_STRFUNC);
...
ssl_common_register_options(module_t *module, ssl_common_options_t *options, bool is_dtls)
{
        ...
        // add trojan secret gui
        prefs_register_filename_preference(module, "trojan_keylog_file", "Trojan-Secret log filename",
            "Trojan keylog file name",
            &(options->trojan_keylog_filename), false);
}
