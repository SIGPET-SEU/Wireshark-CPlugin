...
again:
    ...
    if ((msp = (struct tcp_multisegment_pdu *)wmem_tree_lookup32(flow->multisegment_pdus, seq))) {
        ...
        if (msp->first_frame == pinfo->num) {
            ...
            if (msp->last_frame == pinfo->num) {
                ...
                //col_set_str(pinfo->cinfo, COL_INFO, "[1493 TLS segment of a reassembled PDU]");
                col_append_str(pinfo->cinfo, COL_INFO, "[1493 TLS segment of a reassembled PDU]");
            }
        } else {
    ...
    if (!called_dissector || pinfo->desegment_len != 0) {
        ...
        if (!another_segment_in_frame && pinfo->desegment_offset == 0) {
            ...
            col_set_str(pinfo->cinfo, COL_PROTOCOL,
                    val_to_str_const(session->version, ssl_version_short_names, "SSL"));
            //col_set_str(pinfo->cinfo, COL_INFO, "[1873 TLS segment of a reassembled PDU]");
            col_append_str(pinfo->cinfo, COL_INFO, "[1873 TLS segment of a reassembled PDU]");
            //printf("[1873 TLS segment of a reassembled PDU]\n");
        }
